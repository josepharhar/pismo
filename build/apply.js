...
function setFileTime(absolutePath, filetime) {
    nanoutimes.utimesSync(absolutePath, 
    /* atimeS */ null, 
    /* atimeNs */ null, filetime.mtimeS, filetime.mtimeNs);
}
/**
        ...
        switch (operator) {
            ...
            case 'cp':
                if (srcBranch.remote() && destBranch.remote()) {
                    ...
                    await srcRemote.copyFileFromRemote(srcBranch.name(), srcRelativePath, absoluteLocalDestPath);
                    const srcFileTime = await srcRemote.getRemoteFileTime(srcBranch.name(), srcRelativePath);
                    ...
                    await destRemote.copyFileToRemote(destBranch.name(), destRelativePath, absoluteLocalSrcPath);
                    const srcFileTime = getFileTime(absoluteLocalSrcPath);
                    await destRemote.setRemoteFileTime(destBranch.name(), destRelativePath, srcFileTime);
                }
                else {
                    // local copy
