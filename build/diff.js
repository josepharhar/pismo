...
exports.diff = async function (argv) {
    ...
    if (otherTree) {
        pismoutil.logColor(pismoutil.Colors.bright, `pismo diff ${baseBranch.rawString()} ${otherBranch.rawString()}`
            + `\n + ${baseTree.path}`
            + `\n - ${otherTree.path}`);
        ...
        if (second) {
            ...
            for (const prop in readableFileInfo) {
                if (fileInfo[prop] !== second.fileInfo[prop]) {
                    pismoutil.logColor(pismoutil.Colors.yellow, `  + ${prop}: ${readableFileInfo[prop]}`);
                    pismoutil.logColor(pismoutil.Colors.yellow, `  + ${prop}: ${secondReadableFileInfo[prop]}`);
                }
            }
        }
        else if (treeFile === baseTree) {
            pismoutil.logColor(pismoutil.Colors.green, `+ ${fileInfo.path}`);
        }
        else if (treeFile === otherTree) {
            pismoutil.logColor(pismoutil.Colors.red, `- ${fileInfo.path}`);
        }
        else {
