 ...
 * @param {!events.EventEmitter} cancelEmitter
 */
async function scanPath(relativePathToScan, addPathToScan, addFileInfo, basepath, fileinfoCache, cancelEmitter) {
...
exports.updateInternal = async function (name, nocache) {
    ...
    while (pathsToScan.length && !gotSigint) {
        try {
            await scanPath(pathsToScan.pop(), newPathToScan => pathsToScan.push(newPathToScan), newFileInfo => newTreefile.files.push(newFileInfo), basepath, fileinfoCache, cancelEmitter);
        }
        catch (error) {
