...
class Remote {
    ...
    async copyFileToRemote(treeName, relativePath, absoluteLocalPath) {
        ...
        await api.PutFile.upload(this, putId, fileReadStream.pipe(progressStream), contentLength);
    }
    /**
     * @param {string} treeName
     * @param {string} relativePath
    ...
    copyFileFromRemote(treeName, relativePath, absoluteLocalPath) {
        return new Promise(async (resolve, reject) => {
            ...
            requestReadStream
                .on('error', error => {
                reject(new pismoutil.ErrorWrapper(error, `Failed to download file from remote. treeName: ${treeName}, relativePath: ${relativePath}, absoluteLocalPath: ${absoluteLocalPath}`));
            })
                .pipe(fileWriteStream);
            fileWriteStream.on('error', error => {
                reject(new pismoutil.ErrorWrapper(error, `Failed to write file downloaded from remote. treename: ${treeName}, relativePath: ${relativePath}, absoluteLocalPath: ${absoluteLocalPath}`));
            });
